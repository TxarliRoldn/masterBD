...
p1 <- train_ts %>%
    ...
    autoplot(CO) +
    autolayer(forecast(prophet_CO,   test_ts),  CO,  colour = colors["CO"]) +
    autolayer(test_ts, CO,   color = "#838B8B", alpha = 0.75) +
    labs(x = "", y = "Carbon Monoxide")
p2 <- train_ts %>%
    filter(year(Date) == 2005) %>% 
    autoplot(C6H6) +
    autolayer(forecast(prophet_C6H6, test_ts), C6H6, colour = colors["C6H6"]) +
    autolayer(test_ts, C6H6, color = "#838B8B", alpha = 0.75) +
    labs(x = "", y = "Benzene")
p3 <- train_ts %>%
    filter(year(Date) == 2005) %>% 
    autoplot(NOy) +
    autolayer(forecast(prophet_NOy,  test_ts), NOy,  colour = colors["NOy"]) +
    autolayer(test_ts, NOy,  color = "#838B8B", alpha = 0.75) +
    labs(x = "", y = "Nitrogen Oxides")
p4 <- train_ts %>%
    filter(year(Date) == 2005) %>% 
    autoplot(NO2) +
    autolayer(forecast(prophet_NO2,  test_ts), NO2,  colour = colors["NO2"]) +
...
train_ts %>%
    filter(year(Date) == 2005) %>% 
    autoplot(Temp) +
    autolayer(forecast(tslm_Temp, test_ts), Temp, colour = colors["Temp"]) +
...
train_ts %>%
    filter(year(Date) == 2005) %>% 
    autoplot(TempM) +
    autolayer(forecast(predict_model, test_ts), TempM, colour = colors["TempM"]) +
...
data_ts %>% 
    autoplot(Temp) +
    autolayer(forecast_multi, TempM, color = colors["TempM"]) +
    labs(x = "", y = "Temperature (multi)")
```
